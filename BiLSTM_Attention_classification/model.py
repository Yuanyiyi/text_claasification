...
#     def __init__(self, model,sequence_length=150):
#         self._model = model
#         self._graph = tf.Graph()

#         with self._graph.as_default():
#             self._urlchara = tf.placeholder(tf.int32,[None,model.sequence_length],name='urlcharac')
#             self._logits = self._model.call(self._urlchara)
#             self._probabilities = tf.nn.sigmoid(self._logits)

#             saver = tf.train.Saver()

#         self._session = tf.Session(graph=self._graph)
#         saver.restore(self._session, self._model.ckpt)

#     def __del__(self):
#         self._session.close()

#     # 下面根据具体html页面解析，然后实现预测

#     @utils.timing()
#     def predict(self, html, max_document_length=400):
#         markups = utils.parse(html) if isinstance(html, str) else html
#         try:
#             url = gethtml(markups) # 在data.py中写一个获取html网页中url链接的函数
#         except ValueError:
#             return False, []
#         else:
#             probabilities, scores = self._session.run([self._probabilities, self._similarity],
#                                                         feed_dict={self._urlchara: [url]})

#             top_k = [("".join(tokens), []) for _, tokens in sorted(zip(scores[0], url), reverse=True)[: 5]]
#             return float(probabilities[0][0]) >= 0.85, top_k

#     def predict_prob(self, html, max_document_length=400):
#         """"""
...
#             evidence = [(" ".join(sentence).strip(), score) for sentence, score in zip(url, scores[0])]
#             return probabilities[0], evidence
